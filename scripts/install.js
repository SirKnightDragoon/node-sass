...
function checkAndDownloadBinary() {
  ...
  if(downloads.length){
    ...
    for(var i = 0; i < downloads.length; i++){
      ...
      download('https://github.com/sass/node-sass/releases/download/'+pkg.version+'/'+downloads[i]+'_binding.node', path.join(sass.getBinaryDir(), downloads[i].replace(/_(?=binding\.node)/, '/')), function(err) {
        if (err) {
          console.error(err);
          return;
        }
